...
Deno.serve(async (req) => {
  ...
  if (req.method === 'POST') {
    ...
    // Mark token as used
    await supabase.from('passing_confirmations').update({
      token_used:   true,
...
<body><div class="box"><h2>Solace Life</h2><p>${errorMsg}</p></div></body></html>`
  ...
  return `<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>Confirm Passing — Solace Life</title>
<style>
...
<div class="box">
  ...
  <div class="warn">Only proceed if you have confirmed this person has passed away. Their family will be notified immediately.</div>
  <button class="btn" onclick="doConfirm()">Confirm & Release Vault</button>
...
async function doConfirm() {
  ...
  try {
    ...
    if (data.success) {
      ...
      btn.disabled = false;
      btn.textContent = 'Confirm & Release Vault';
    }
  } catch(e) {
